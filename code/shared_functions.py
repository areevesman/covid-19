...
#                 [{'label': location, 'value': '/countries/' + location.replace(' ', '').lower()} \
#                  for location in world_dropdown_choices] + \
#                 [{'label': location, 'value': '/states/' + location.replace(' ', '').lower()} \
...
#         elif pathname.lower()[:10] == '/countries/unitedstates':
#             location_colname = 'State'
#             location_colname = 'unitedstates'
#         totals = cummulative_cases[cummulative_cases['Date'] == dates[day]][[location_colname, 'Cases', 'Deaths']] \
#             .sort_values('Cases', ascending=False)
#         print(totals.head())
#         totals['Cases'] = totals['Cases'].map(lambda x: f'{x:,}')
#         totals['Deaths'] = totals['Deaths'].map(lambda x: f'{x:,}')
#         totals['Links'] = ['/states/' + state for state in totals[location_colname]]
